...
var FaithComesByHearingAudio = (function() {
	...
	function loadFcbHearingLocations() {
		$.ajax({
			dataType: 'jsonp',
			url: sofia.config.dbpBaseUrl + 'audio/location?v=2&reply=jsonp&key=' + sofia.config.fcbhKey,
	...
	function loadFcbHearingInfo() {
		$.ajax({
			dataType: 'jsonp',
			url: sofia.config.dbpBaseUrl + 'library/volume?v=2&reply=jsonp&media=audio&delivery=web&key=' + sofia.config.fcbhKey,
			success: function(data) {
				fcbhList = data;
