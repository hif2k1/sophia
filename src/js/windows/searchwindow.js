...
var SearchWindow = function(id, parent, init_data) {
	...
	var header = $('<div class="window-header search-header" >' +
						...
						'<div class="search-options-button header-icon" style=""><svg class="icon"><use xlink:href="build/icons.svg#settings-button"></use></svg></div>' +
	...
	function searchCompleteHandler(e) {
		...
		if (results && results.length > 0) {
			...
			for (var i=0, il=strongsKeys.length; i<il; i++) {
				strongsHtml += '<option value="' + strongsKeys[i] + '">' + strongsKeys[i] + ' | ' + strongsUsage[strongsKeys[i]].count + '</option>';
			}
			if(strongsKeys.length != 0) {
				topUsage.append('<select id="strongs-matches"><option> -- Strongs -- </option>' + $(strongsHtml) + '</select>' ).show();

				$("#strongs-matches").change(function() {
					$('.search-text').val($( "#strongs-matches option:selected").val());
					doSearch();
				});
